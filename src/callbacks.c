...
void search_closest_tank(World *world, Vertex camera_position)
{
	int tank_id;
	...
	{
		float distance = sqrt(pow((camera_position.x - world->tanks[i].position.x), 2) + pow((camera_position.z - world->tanks[i].position.z), 2));
...
void keyboardUp(unsigned char key, int x, int y)
{
	...
	case 'x':
		if (move_tank_id == -1)
		{
			search_closest_tank(&world, camera.position);
		}
		else
		{
			move_tank_id = -1;
	...
	case 'q':
		if (move_tank_id != -1)
		{
			world.tanks[move_tank_id].rotation += 10.0f;
		}
		break;
	case 'e':
		if (move_tank_id != -1)
